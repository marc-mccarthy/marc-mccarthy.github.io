			...
			futureToDos.length === 0 ? (
				...
				<Page>
					<Content>
						<Category>
							...
							<TechList>
								{technologies.map((tech, index) => {
									return (
										<TechItem key={index}>
											<Anchor href={tech.link} target="_blank" rel="noopener">
												<BadgeLink
													src={tech.logo}
													alt={tech.tech_name}
												></BadgeLink>
						...
						<Category>
							<TechTitle>Services Used:</TechTitle>
							<TechList>
						...
						<Category>
							<TechTitle>Future To-Do's:</TechTitle>
							<ToDoList>
								{futureToDos.map((todo, index) => {
									return (
										<ToDoItem key={index}>
											<input type="checkbox" id={index} />
											<label for={index}>{todo.future_to_do}</label>
										</ToDoItem>
									);
								})}
							</ToDoList>
